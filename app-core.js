...
async function loadComponent(path) {
  ...
  try {
    ...
    return `<div class="error">UI failed to load. Refresh or check deployment.</div>`;
  }
}

async function initProductionApp() {
  try {
    ...
    document.getElementById('app').innerHTML = `
      <div class="error">Failed to load UI. Please refresh. ${error.message}</div>
    `;
  }
}
...
if (document.readyState === 'complete') {
  ...
  window.addEventListener('DOMContentLoaded', initProductionApp);
}
